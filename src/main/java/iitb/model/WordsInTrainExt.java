...
public class WordsInTrainExt implements Serializable {
    ...
    public WordsInTrainExt() {
	this(new TokenGenerator());
    }
    public WordsInTrainExt(TokenGenerator tokenGen) {
	...
	for (trainData.startScan(); trainData.hasNext();) {
		...
		for (tokenGenerator.startScan(seq.x(l)); tokenGenerator.hasNext();) {
                    addDictElem(tokenGenerator.next().toString(),seq.y(l),numStates);

                    if(seq.x(l).getClass().equals(SSFLexItem.class))
                        addDictElem(((SSFNode) tokenGenerator.next()).getName(),seq.y(l),numStates);
		}
	    }
	}
