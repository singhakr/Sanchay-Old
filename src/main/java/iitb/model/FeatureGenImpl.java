...
public class FeatureGenImpl implements FeatureGeneratorNested {
    ...
    protected transient Iterator featureIter;
    protected FeatureTypes currentFeatureType;
    protected FeatureImpl featureToReturn,  feature;
    public Model model;
    protected int numFeatureTypes = 0;
    protected int totalFeatures;
    protected boolean _fixedTransitions = true;
    public boolean generateOnlyXFeatures = false;
    public boolean addOnlyTrainFeatures = true;
    protected TIntHashSet retainedFeatureTypes = new TIntHashSet(); // all features of this type are retained.
    protected transient DataSequence data;
    protected int cposEnd;
    protected int cposStart;
    protected WordsInTrainExt dict;
    ...
    public void setDict(WordsInTrainExt d) {
        dict = d;
    }

    public WordsInTrainExt getDict() {
        if (dict == null) {
    ...
    protected boolean retainFeature(DataSequence seq, FeatureImpl f) {
        return ((seq.y(cposEnd) == f.y()) && ((cposStart == 0) || (f.yprev() < 0) || (seq.y(cposStart - 1) == f.yprev())));
    }
