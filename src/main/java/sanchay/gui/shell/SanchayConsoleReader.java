 ...
 * @author anil
 */
public class SanchayConsoleReader implements ConsoleOperations {
     ...
     * The Terminal to use.
     */
    private final SanchayTerminal terminal;

//    private CompletionHandler completionHandler = new CandidateListCompletionHandler();
    private CompletionHandler completionHandler = new SanchayCompletionHandler();
...
//    InputStream in;
     ...
     */
    public SanchayConsoleReader() throws IOException {
     ...
     */
    public SanchayConsoleReader(final InputStream in, final Writer out)
            throws IOException {
        this(in, out, null);
    }
    ...
    public SanchayConsoleReader(final InputStream in, final Writer out,
        ...
        this(in, out, bindings, (SanchayTerminal) SanchayTerminal.getTerminal());
    }
     ...
     *            the terminal to use
     */
    public SanchayConsoleReader(InputStream in, Writer out, InputStream bindings,
            SanchayTerminal term) throws IOException {
        this.terminal = term;
    ...
    public SanchayTerminal getTerminal() {
    ...
    public void setDebug(final PrintWriter debugger) {
        SanchayConsoleReader.debugger = debugger;
    }
    ...
    public String readLine(final String prompt, final Character mask)
        ...
        try {
		    ...
		    break;
                ...
                case CLEAR_LINE:
                    moveInternal(-(buf.getBuffer().length()));
                    killLine();
                    break;
...
//    private String readLine(JTextArea inTextArea) throws IOException {
...
//    }
    ...
    public boolean killLine() throws IOException {
        int cp = buf.cursor;
        int len = buf.getBuffer().length();

        if (cp >= len) {
            return false;
        }

        int num = buf.getBuffer().length() - cp;
        clearAhead(num);

        for (int i = 0; i < num; i++) {
            buf.getBuffer().deleteCharAt(len - i - 1);
        }
    ...
    private final boolean complete() throws IOException {
        ...
        String bufstr = buf.getBuffer().toString();
        ...
        return ((SanchayCompletionHandler) completionHandler).complete(this, candidates, position);
    }
    ...
    private final void setBuffer(final String buffer) throws IOException {
        ...
        if (buffer.equals(buf.getBuffer().toString())) {
            return;
        }
        ...
        for (int i = 0, l1 = buffer.length(), l2 = buf.getBuffer().length(); (i < l1)
                && (i < l2); i++) {
            if (buffer.charAt(i) == buf.getBuffer().charAt(i)) {
                sameIndex++;
            } else {
                break;
            }
        ...
        int diff = buf.getBuffer().length() - sameIndex;

        backspace(diff); // go back for the differences
        killLine(); // clear to the end of the line
        buf.getBuffer().setLength(sameIndex); // the new length
    ...
    public final void drawLine() throws IOException {
        ...
        printString(buf.getBuffer().toString());
    ...
    final String finishBuffer() {
        String str = buf.getBuffer().toString();
    ...
    private final void drawBuffer(final int clear) throws IOException {
        ...
        char[] chars = buf.getBuffer().substring(buf.cursor).toCharArray();
        if (mask != null)
    ...
    private final int backspace(final int num) throws IOException {
        ...
        buf.getBuffer().delete(buf.cursor, buf.cursor + count);
        drawBuffer(count);
    ...
    private final boolean deleteCurrentCharacter() throws IOException {
        boolean success = buf.getBuffer().length() > 0;
        if (!success) {
            return false;
        }

        if (buf.cursor == buf.getBuffer().length()) {
            return false;
        }

        buf.getBuffer().deleteCharAt(buf.cursor);
        drawBuffer(1);
        return true;
    }
    ...
    public final int moveCursor(final int num) throws IOException {
        ...
        if ((buf.cursor == buf.getBuffer().length()) && (where > 0)) {
            return 0;
        }

        if ((buf.cursor + where) < 0) {
            where = -buf.cursor;
        } else if ((buf.cursor + where) > buf.getBuffer().length()) {
            where = buf.getBuffer().length() - buf.cursor;
        }
    ...
    private final void moveInternal(final int where) throws IOException {
	    ...
	    return;
            ...
            printCharacters(buf.getBuffer().substring(buf.cursor - where, buf.cursor).toCharArray());
            return;
        }
    ...
    throws IOException
    {
        ...
        buf.getBuffer().delete (buf.cursor, buf.cursor + 1);
        drawBuffer (1);
    ...
    public final boolean replace(int num, String replacement) {
        buf.getBuffer().replace(buf.cursor - num, buf.cursor, replacement);
        try {
            moveCursor(-num);
