...
import sanchay.util.query.ProcessQuery;
 ...
 */
public class SVMExtractFeaturesByQuery
{
...
//protected ContextFeaturesImpl contextFeatures = new ContextFeaturesImpl();
    ...
    private boolean []featuresUsed;
    private ProcessQuery pQuery;
    private String []featureQueryList;
    private int queryCount;
    protected final int MAXQUERYCOUNT=19;
    ...
    public void init(SSFStoryImpl ssfStory, SSFSentenceImpl sentence, SSFNode currentWord, String cs, MLClassLabels MLLabel)
    {
        ...
        queryCount=0;
        CONFIG_FILE_PATH="/home/l53590/IIITH/Sanchay_15_12_09/Sanchay/workspace/featuresConfig.txt";
        setFeatureQueryList(CONFIG_FILE_PATH);
        pQuery=new ProcessQuery();
    }
    
    private double assignWeight(String featureDetails) {
        //Assign weight according to the feature Details obtained.
        boolean first=true;
        double wt= 1.0;
        int l = featureDetails.length(),i;
        char ch;
        for(i=0;i<l;i++){
            ch=featureDetails.charAt(i);
    ...
    private void setFeatureQueryList(String CONFIG_FILE_PATH) {
        try {
            FileInputStream fstream;
            fstream = new FileInputStream(CONFIG_FILE_PATH);
            DataInputStream in = new DataInputStream(fstream);
            BufferedReader br = new BufferedReader(new InputStreamReader(in));
            String strLine;
            int i = 0;
            featureQueryList= new String[MAXQUERYCOUNT];
            while ((strLine = br.readLine()) != null)   {
                queryCount++;
                featureQueryList[i++]=strLine;
            }
            in.close();
        } catch (Exception ex) {
            Logger.getLogger(SVMExtractFeaturesByQuery.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        protected List generalFeatrue(String extractedFeature, double  wt){
        //System.out.println("In GeneralFeature()"+"extractedFeature="+extractedFeature);
        List feature = new ArrayList();
        SVMFeatureNode svmNode = new SVMFeatureNode();
        int label=-1;
        label = getLabel(extractedFeature);
        ...
        public void configOptions(){
            int i=0;
            double wt;
            String queryOutput= new String();
            String featureDetails= new String();
            String op= new String();
            
            //System.out.println(" Query Count:"+queryCount);
            for(i=0;i<queryCount;i++){
                op=pQuery.nodeNvalueWithFeatureDetails(word,featureQueryList[i],true);
                featureDetails=pQuery.featureDetails;
                wt=assignWeight(featureDetails);
                //wt=1.0;
                //System.out.println(" Calling GeneralFeature()");
                queryOutput=featureDetails+op;
                //System.out.println("output of processQuery.nodeNvalue()="+queryOutput);
                
                if(op!=null&&!op.equals("null")){ 
                    //System.out.print("[Output: "+queryOutput+" ("+wt+")]");
                    generalFeatrue(queryOutput, wt);
                }
            }
    ...
    {
        int i=0;
        if(featuresUsed[i++])addFeature( quantifierFeature(3.5) );
        if(featuresUsed[i++])addFeature( symbolFeature(3.5) );
        if(featuresUsed[i++])addFeature( wordLength(1) );
        if(featuresUsed[i++])addFeature( suffixFeature(2.8) );
        if(featuresUsed[i++])addFeature( prefixFeature(2.7) );
        if(featuresUsed[i++])addFeature( FSFeature("cat", word) );
        if(featuresUsed[i++])addFeature( FSFeature("gend", word) );
        if(featuresUsed[i++])addFeature( FSFeature("num", word) );
        if(featuresUsed[i++])addFeature( FSFeature("pers", word) );
        if(featuresUsed[i++])addFeature( FSFeature("case", word) );
        if(featuresUsed[i++])addFeature( FSFeature("vib", word) );
        if(featuresUsed[i++])addFeature( FSFeature("tam", word) );
        if(featuresUsed[i++])addFeature( previousWordsFeature(1 /*getWINDOW_SIZE()*/,2.6)  );
        if(featuresUsed[i++])addFeature( nextWordsFeature(3 /*getWINDOW_SIZE()*/,2.6)  );
        if(featuresUsed[i++])addFeature( previousWordsPOSFeature(getWINDOW_SIZE()) );
        if(featuresUsed[i++])addFeature( currentWord(3.0) );
       // if( TESTING_MODE == 0 )
       // {
       //    addFeature(PosProbabilities());
       // }
        //addFeature(rareWord());                                           // Not impleneted - need the "frequent words" list
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "cat") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "gend") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "num") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "pers") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "case") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "vib") );
        if(featuresUsed[i++])addFeature( PreviosWordsFSFeature(getWINDOW_SIZE(), "tam") );
     }
